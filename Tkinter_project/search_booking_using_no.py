...
root.title('View Your Booking detail')
bg=PhotoImage(file='ave.png')
bg_img=Label(root,image=bg)
bg_img.place(x=0,y=0,relwidth=1,relheight=1)
top_frame=Frame(root,width=900,bd=1)
top_frame.pack(side=TOP)
titlename=Label(top_frame,text='View Your Booking detail',bg='red',fg='white',font=('Helvetica', 18))
...
Number=Label(miframe,text='Enter your Mobile Number',bg='orange',fg='white',font=('Helvetica', 15))
Number.grid(row=0,column=0,padx=10,pady=10)

NumberE=Entry(miframe,textvariable=number)
NumberE.grid(row=0,column=1,padx=10,pady=10)
...
view=ttk.Treeview(mid_frame,columns=('name','number','brand','year','month','amount'),selectmode = 'extended')
...
def search():
    ...
    cursor=conn.cursor()
    cursor.execute(""" select * from 'booking_tbl' where number=?""",(number.get(),))
    conn.commit()
...
searchbtn=Button(miframe,text='SEARCH',bg='lightblue',fg='purple',font=('Helvetica', 15),command=search,cursor="hand2")
searchbtn.grid(row=4,column=1,padx=10,pady=10)
